...
1. **COCKPIT PREPARATION** — BAT 1+2 ON → EXT PWR ON → ADIRS NAV → NAV & LOGO ON → SIGNS ON → MCDU INIT(FROM/TO 入力 例: `RJTT/RJOO` → LSK1R)
2. **BEFORE START** — FUEL PUMPS ON → APU MASTER ON → APU START(AVAIL まで約 25 秒)→ APU BLEED ON → PARK BRK 確認 → BEACON ON
3. **ENGINE START** — ENG MODE SEL を IGN/START → ENG MASTER **2** ON(N2 安定まで約 40 秒)→ ENG MASTER **1** ON → MODE SEL を NORM
4. **AFTER START** — APU BLEED OFF → APU MASTER OFF → EXT PWR OFF
5. **SHUTDOWN & SECURING** — PARK BRK ON → EXT PWR ON → ENG MASTER 1+2 OFF → BEACON OFF → SEAT BELTS OFF → FUEL PUMPS OFF → ADIRS OFF → NAV & LOGO OFF → EXT PWR OFF → BAT 1+2 OFF(Cold & Dark に戻る)
